...
## ADR-17 · `SoundConfig` split + optional separate break sound (additive, not a schema bump)
**Context:** users wanted a *different* background sound during breaks when audio isn't muted. The audio model was one flat sound on `AudioSettings`, and `AudioController` consumed the whole settings object.
**Decision:** factor out **`SoundConfig`** = the playable part (category + `noise`/`binaural`/`media` designs + its own `volume`). `AudioSettings` **extends** `SoundConfig` (so the *focus* sound stays flat — minimal churn for existing readers) and adds `pauseOnBreak` plus an optional **`break: SoundConfig | null`** (null = keep the focus sound through breaks — preserves prior behavior). The picker UI splits into a reusable **`SoundPicker`** (one `SoundConfig`) composed twice by `AudioPicker` (focus + break); the break picker only shows when `pauseOnBreak` is off, and enabling it seeds `break` from the current focus sound (`soundOf`). `AudioController` now takes the **effective** `SoundConfig` and `RunScreen` chooses focus-vs-break (`useBreakSound = onBreak && !pauseOnBreak && break !== null`). Kept the persisted version at **v2** — `break` is purely additive and `migrateAudio`'s default-merge backfills it to `null`, so no migration bump was needed.
**Consequences:** independent break music (any category, incl. media) reusing all the designers/preview wiring; the controller is simpler (one sound, not a settings object). A focus⇄break swap is handled by the existing live design effects rather than a dedicated crossfade, so a same-category swap morphs smoothly while a cross-category or to/from a different track may click at the boundary — acceptable for a break transition; revisit with a two-bus crossfade if it grates. Persisted v2 configs with a `break` set will look unknown to any *older* build that predates this change (it would ignore the field), which is fine.
