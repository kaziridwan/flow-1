...
for each session i in 1..N:
  for meal in [breakfast, lunch, dinner]:
    if enabled and not taken and
       (mealSlots[meal] == i  OR  (mealSlots[meal] unset and cursorTimeOfDay ∈ window)):
...
- Manual placement: `cfg.mealSlots[meal]` (set by dragging a meal onto a focus block in the preview, ADR-20) pins that meal right before the given focus session, **overriding its clock window**. Unpinned meals keep the window behavior. A slot beyond the session count simply never places.
...
- `playBinauralTrack(design, offsetSec=0)` — two sine oscillators panned hard left/right via a `ChannelMerger` (left=base, right=base+beat). Each segment follows the **timing function** of its starting keyframe (`transition`, ADR-21): continuous easings (`linear`/`ease`/…/`cubic-bezier`) are sampled into ~24 short `linearRampToValueAtTime` sub-ramps; `step-start`/`step-end` are scheduled as `setValueAtTime` jumps. `offsetSec` **seeks** into the track: the value at the offset is set immediately (`interpolateBinaural`, itself ease-aware) and only later samples ramp. Looping is a `setTimeout(durationSec − offset)` that snaps back to t=0, then a `setInterval(durationSec)`, so the audio stays in phase with the editor's preview cursor (which wraps at the same instant). Idempotent — re-call to reschedule/seek on edit without restarting the oscillators. See ADR-12, ADR-16, ADR-21.
...
Binaural **timing functions** live in `src/lib/easing.ts` (pure): `easeProgress(spec, p)` maps linear progress through a keyword or `cubic-bezier(...)` curve (CSS-accurate Newton/bisection solver); `isValidTiming`/`parseCubicBezier` validate user input (x control points must be in [0,1]); `easingCurvePoints` feeds the editor's curve preview. `BUILTIN_BINAURAL_PRESETS` (in `audioDesign.ts`) holds the ready-made keyframed tracks, and `resolveFreqEdit(left, diff, lock, field, value)` implements the Left>Right>Diff edit/lock precedence — both unit-tested (ADR-21).
...
- **YouTube**: lazy-loads the IFrame API once (`loadYouTubeApi`), creates/destroys a player for the parsed video id (`youtubeId()` in `format.ts`), and play/pauses + sets volume on the `active`/volume effects. Because the API loads asynchronously (after the Start gesture), the player is created with `autoplay` set to the mount-time play state and `onReady` enforces play/pause from `activeRef` (the live `active`), so media **autoplays once the run is playing** rather than reading a stale closure value.
- **Podcast / media**: a hidden `<audio>` element (`preload="auto"`, `playsInline`); `play()/pause()` and `volume` via effects keyed on `active`/url.
- **Autoplay caveat:** browser policy allows this off the Start click (a user gesture). A *break* sound that's YouTube/media starting mid-run has no fresh gesture, so the browser may withhold autoplay there.

**Saved presets** live in `src/lib/presetStore.ts` — a tiny `localStorage`-backed external store (`subscribe` + snapshot, read via `usePresets()` / `useSyncExternalStore`) rather than threaded through props, since presets are read/written deep in the tree (designers and `SoundPicker`). It holds named `SavedBinauralPreset[]` / `SavedNoisePreset[]`; mutators (`add*Preset`, `deletePreset`, `renamePreset`) persist and notify synchronously. The shared `PresetControls` component renders the save/apply/rename/delete UI and is mounted inside the two designer sheets (Noise Designer / Binaural Engine), so presets are managed where the design is edited (ADR-18).
